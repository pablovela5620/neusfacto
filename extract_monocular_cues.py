...
def save_outputs(img_path, output_file_name):
    with torch.no_grad():
        save_path = os.path.join(
            args.output_path, output_file_name.replace("_rgb", f"_{args.task}") + ".png"
        )
        ...
        if args.task == "depth":
            if scale_factor > 1:
                output = F.interpolate(
                    output.unsqueeze(0), scale_factor=scale_factor, mode="nearest"
                ).squeeze(0)
            ...
            if scale_factor > 1:
                output = torch.nn.functional.interpolate(
                    output, scale_factor=scale_factor, mode="nearest"
                )
            np.save(save_path.replace(".png", ".npy"), output.detach().cpu().numpy()[0])
